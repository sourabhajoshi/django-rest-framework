...
A Django Model Manager is used to talk to the database. It helps you get data, filter records, create new entries, update, and delete data.

Model Manager ಅಂದರೆ model ಮತ್ತು database ನಡುವಿನ bridge.
ಈದು database query ಗಳನ್ನು (filter, create, update, delete) ಮಾಡಲು ಬಳಸಲಾಗುತ್ತದೆ.

Every model has a default manager called objects.

We can also create custom managers to write special queries.

### Why is it useful

Because it keeps database logic separated from business logic, giving a clean structure and reusable querying methods.
...
class StudentManager(models.Manager):
    def adults(self):
        return self.filter(age__gte=18)  # Fetch only 18+ students
...
Usage
Student.objects.adults()  # Returns all students aged 18 and above
```

## 17. Explain the purpose of the Meta class in Django models.

The Meta class is an inner class inside a Django model that is used to give extra information about the model.

It does NOT create fields — instead, it controls how the model behaves in the database and admin.

Think of Meta as the settings section for a model.

Meta class ಅಂದ್ರೆ model ಒಳಗೆ ಇರುವ settings class. Fields ಸೃಷ್ಟಿ ಮಾಡೋದಿಲ್ಲ — model ಹೇಗೆ ಕೆಲಸ ಮಾಡಬೇಕು ಅನ್ನೋದು control ಮಾಡುತ್ತದೆ.
- db_table → Table ಹೆಸರು
- ordering → Default sorting
- unique_together → Duplicate data ತಡೆಯಲು

### Why is it useful

- To make database tables more meaningful
- To control how data appears in queries/admin
- To reduce repeated ordering/filtering code
- To apply model rules in one place

| Situation                           | Meta Solution     |
| ----------------------------------- | ----------------- |
| Want custom table name              | `db_table`        |
| Always show students A→Z            | `ordering`        |
| Prevent duplicate email+phone users | `unique_together` |
| Show better names in admin          | `verbose_name`    |
...
class Student(models.Model):
    ...
    class Meta:
        db_table = "student_records"           # Custom table name
        ordering = ["name"]                    # Sort by name A-Z
        verbose_name = "Student Profile"       # Display name in admin
        unique_together = ["name", "email"]    # No duplicates allowed
```

## 18. How do you perform database queries using Django’s ORM?

Django ORM (Object Relational Mapper) lets you interact with the database using Python code instead of SQL.

This means we can create, read, update, and delete data using Python methods — no need to write raw SQL manually.

(ORM = Python code → SQL automatically handled)

### Key Points
| Action          | ORM Method                      | Purpose            |
| --------------- | ------------------------------- | ------------------ |
| Read data       | `.all()`, `.get()`, `.filter()` | Fetch data         |
| Create data     | `.create()` , `.save()`         | Insert into DB     |
| Update data     | `.update()`                     | Modify records     |
| Delete data     | `.delete()`                     | Remove records     |
| Count           | `.count()`                      | Number of items    |
| Sorting         | `.order_by()`                   | Sort results       |
| Check existence | `.exists()`                     | Returns True/False |

### Basic Query Examples

| Operation             | ORM Query                                      | Description                                   |
| --------------------- | ---------------------------------------------- | --------------------------------------------- |
| Get all records       | `Student.objects.all()`                        | Fetch every student from the database         |
| Filter records        | `Student.objects.filter(age=20)`               | Students with age = 20                        |
| Filter with condition | `Student.objects.filter(age__gte=18)`          | Students aged **18 or above**                 |
| Get single record     | `Student.objects.get(id=1)`                    | Fetch record with ID 1 *(error if not found)* |
| Order by Ascending    | `Student.objects.order_by('age')`              | Sort by age (low → high)                      |
| Order by Descending   | `Student.objects.order_by('-age')`             | Sort by age (high → low)                      |
| Limit records         | `Student.objects.all()[:5]`                    | First 5 records                               |
| Exclude records       | `Student.objects.exclude(age=18)`              | All except age 18                             |
| Count total records   | `Student.objects.count()`                      | Number of records                             |
| Check record exists   | `Student.objects.filter(name="Ravi").exists()` | True/False if record found                    |
| Create new record     | `Student.objects.create(name="Ravi", age=20)`  | Add a new student                             |
| Update record         | `Student.objects.filter(id=1).update(age=25)`  | Change age to 25                              |
| Delete record         | `Student.objects.get(id=1).delete()`           | Remove record permanently                     |


## 19. Difference between select_related() and prefetch_related()

When we fetch related data from multiple tables (ForeignKey, ManyToMany), Django can make too many database queries, which slows down performance.

To solve this, Django provides:

- select_related() → Faster for ForeignKey / One-To-One (1:1, Many:1)

- prefetch_related() → Faster for Many-To-Many / Reverse ForeignKey

### Key Difference (Very Simple)
| Feature      | `select_related()`             | `prefetch_related()`                     |
| ------------ | ------------------------------ | ---------------------------------------- |
| Best for     | ForeignKey, One-To-One         | Many-To-Many, Reverse FK                 |
| How it works | SQL Join (single query)        | Multiple queries + join in Python memory |
| Speed        | Faster for small relationships | Faster for big related datasets          |
| Returns      | Single optimized query         | Multiple optimized queries               |
| Use Case     | Each Book → One Author         | Author → Many Books                      |
...
A database transaction is a process where multiple database operations are treated as one single unit of work.

- If all steps succeed, the data is saved.
- If any step fails, everything is cancelled/rolled back (database returns to original state).

Transaction = ಎಲ್ಲಾ steps success ಅಂದ್ರೆ save, ಇಲ್ಲದಿದ್ದರೆ rollback.
ಒಂದೇ ಯೂನಿಟ್ ಆಗಿ ಕೆಲಸ ಮಾಡುತ್ತದೆ.

### Why Transactions Are Important
- To avoid wrong data when multiple operations depend on each other
- To protect data when a step fails due to error or power outage
- To guarantee data consistency
- To ensure money-transfer type operations are safe

### Real-World Use Cases
- Bank Money Transfer
- Inventory Update in E-commerce
- Ticket Booking / Seat Reservation
- Multi-Model saving (Order + OrderItems)
...
### Code Example — Using transaction.atomic()
```
...
def transfer_money(sender, receiver, amount):
    with transaction.atomic():
        ...
        # If this fails, whole block will rollback
        receiver.balance += amount
        receiver.save()
```
If any .save() fails → both changes are cancelled and prevent half update.

without transition, Receiver save fails - Sender balance is lost.

## 21. What is a Django Template and how is it rendered?

A Django Template is an HTML file with variables/tags used to display dynamic data.
